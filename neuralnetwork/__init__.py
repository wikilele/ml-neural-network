...
def from_parameters(params):
    ...
    imput_dim = params['input_layer_dim']
    hidden_layer_num = params['hidden_layer_number']
    hidden_layer_dim = params['hidden_layer_dim'] # should be a tuple of x elements where x is the hidden_layer_num
    output_layer_dim = params['output_layer_dim']

    input_layer(input_dim)
    for i in range(hidden_layer_num)
        hidden_layer(hidden_layer_dim[i], activation='sigmoid')
    output_layer(output_layer_dim, activation='sigmoid')
